...
void MaxPoolLayer::createPrimitives(std::vector<size_t> const &src_dimensions,
  ...
  dst_dimensions.push_back(src_dimensions[1]); 
  size_t dst_h = std::ceil(((float) (src_dimensions[2]-kernel_h_+2*padding_h_))/stride_h_)+1;
  size_t dst_w = std::ceil(((float) (src_dimensions[2]-kernel_w_+2*padding_w_))/stride_w_)+1;
